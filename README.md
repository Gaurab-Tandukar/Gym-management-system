# Blackthrone GYM Management System
...
- **Member Control** — activate/deactivate membership, mark attendance, revert members (search by ID)
...
> **Note:** Place `logo in black.png` and `logo in white.png` in the same directory as the compiled classes for the logo to display correctly.
...
> Data is stored in memory during the session. Use **"Save to File"** before closing to persist records.
